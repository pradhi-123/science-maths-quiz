...
export function getSavedQuestions() {
  const data = localStorage.getItem('school_quiz_questions_v21');
  let savedList = [];
  if (data) {
    try {
  ...
  if (Array.isArray(savedList) && savedList.length > 0) {
    ...
    localStorage.setItem('school_quiz_questions_v21', JSON.stringify(merged));
    return merged;
  }

  // Initialize with seed
  localStorage.setItem('school_quiz_questions_v21', JSON.stringify(DEFAULT_QUESTIONS));
  return DEFAULT_QUESTIONS;
}

export function saveQuestions(list) {
  localStorage.setItem('school_quiz_questions_v21', JSON.stringify(list));
}
